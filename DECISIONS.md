...
D-034 · 2026-09-21 · agent (under D-024) · Layla's LAYLA.md Parts 1–2 (pushed to the old `cursor/…` branch, c72af20) copied into `data/persona.json` (3 background lines, 3 memory lines) and `src/prompts.js` (natural-not-checklist line, 2–3 min close); her LAYLA.md brought to `main` with main's web-call Part 3. Not applied: asking whether she "has taken her tablet" (hard rule 3; it caused the medication advice in the first live call); "if Mia is okay, ask whether she needs anyone to come over" (deferred: a "yes" to that question would fight the yes/no outcome rules before freeze); the rename Belletje → "HAVI" (product name, waits for Daniel). · Her deadline content, with the two safety exceptions stated.
...
D-039 · 2026-09-21 · agent (Daniel: "fix it"; follows the Apple SOS anchor, PLAN "The bar" #2) · SOS skips the check call and goes straight to Tom ("She needs help now"); a detected fall still calls Mia first, waits 15 s, then Tom. Supersedes the `fall | sos → call Mia` line of PLAN §6 for SOS only. A second alert during an open incident is still ignored (B11), but the API now says so (`ignored: "… press Reset demo first"`) and the Watch tab shows that text instead of "calling Mia". Tom's call, SMS and alert no longer say "isn't answering" for an SOS. · Live run 17:07: pressing SOS called the person who had just pressed it, and a Fall toggle during that incident said "calling Mia" while the server ignored it. B08c pins it.

D-040 · 2026-09-21 · agent (Daniel reported it live) · Tom's second "no" (after he already declined and the neighbour is ringing) hands over to the alarm centre at once and ends the neighbour call; it no longer re-dials the neighbour. Tom's alert follows the stage: calling Tom → "I'm going now / I can't — try Mr Hendriks"; calling the neighbour → "HAVI is calling Mr Hendriks" + "I can go after all / Hand over to the alarm centre now"; alarm centre → "The alarm centre has the case (simulated in this demo)" + "I'm going now / Mia is safe". · Live run 17:19: declining on the phone page, then tapping "I can't" in the app, called Mr Hendriks twice and the alert kept asking Tom "Can you go now?". B09b pins it.

D-041 · 2026-09-21 · Daniel (Phase 2 gate, option A) · The recurring-complaint flag is pinned by code: "Dizzy 3× this week — suggest calling the GP" (count from memory + this call); the LLM's own dizzy flag is dropped, its other flags stay. · Same line on screen every run; the model had invented "now Thursday". Checked with the real LLM at 17:23.

D-042 · 2026-09-21 · Daniel (Phase 3 gate, option A) · On stage: live web calls. If a live leg fails (Wi-Fi, microphone, Vapi), switch "Live phone calls" off, press Reset demo and rerun: simulated, labelled, everything after the call still real. · Keeps the live moment and a one-switch fallback.

D-043 · 2026-09-21 · Daniel · Feature freeze at 17:25. From here only fixes for failures seen in a dry run; no new features.
